...
function addNewWordSubmission(word) {
    ...
    // var alreadyUsed
    // var alreadyUsed = model.wordSubmissions.forEach(function(submission){
    //
    //     if (submission.word == word){
    //         emptyOrnot=emptyOrnot+1;
    //     }else{
...
function render() {
    ...
    // $("#word-attempt-form").remove(redLetterChips);
    // $("#word-attempt-form").remove(".disallowed-letter");
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        ...
        if (wordSubmission.isRealWord){
            var scoreChip = $("<span></span>").text(wordScore(wordSubmission.word)).addClass("tag tag-sm tag-primary");
        }else{
...
function containsOnlyAllowedLetters(word) {
    ...
    // if disallowedLettersInWord is none
...
function currentScore() {
    ...
    var sumScore=0;
    // sumScore += int(wordScores);
    // return sumScore;
    // var totalwordScores =
    for (var i = 0; i < wordScores.length; i++) {
        sumScore = wordScores[i]+sumScore;
    }
    return sumScore;
}
